...
bool LinkedList<ItemType>::sortInsert(const ItemType& newEntry)
{
    if (itemCount<1)
    ...
    else if(newEntry<getEntry(1))
        return insert(1,newEntry);
    else if (getEntry(1)<newEntry && newEntry<getEntry(itemCount)){
        for (int i=1; i<itemCount; i++){
            if (getEntry(i)< newEntry && newEntry<getEntry(i+1)){
                insert(i+1, newEntry);
                itemCount--;
            }
        }
        ...
        return insert(getLength()+1,newEntry);
